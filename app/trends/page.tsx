  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-xl font-semibold text-amber-900">Trending Topics</h1>
            </div>
            <Button
      ...
      <div className="p-6">
        {/* Header Section */}
        <div className="mb-8">
          <h2 className="text-3xl font-bold mb-2 flex items-center text-amber-900">
          ...
          <div className="flex gap-2 flex-wrap">
            {categories.map((category) => (
              <Button
                ...
                className={`capitalize ${selectedCategory === category ? '' : 'border-orange-200 hover:bg-orange-50'}`}
              >
                {category}
              </Button>
            ))}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, index) => (
              <Card key={index} className="border-0 shadow-sm bg-white/60 backdrop-blur-sm border-orange-200/60 animate-pulse">
                <CardContent className="p-6">
                  <div className="h-4 bg-amber-200 rounded mb-2"></div>
                  <div className="h-3 bg-amber-200 rounded mb-4"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTopics.map((trend, index) => (
              <Card
                key={index}
                className="border-0 shadow-sm bg-white/80 backdrop-blur-sm border-orange-200/60 hover:shadow-md transition-shadow"
              >
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <CardTitle className="text-lg mb-2 flex items-center text-amber-900">
                        <Hash className="mr-2 h-5 w-5 text-orange-600" />
                        {trend.topic}
                      </CardTitle>
                      <CardDescription className="text-sm leading-relaxed text-amber-700">{trend.description}</CardDescription>
                    </div>
                    <Badge variant="secondary" className="ml-2 capitalize bg-amber-100 text-amber-800">
                      {trend.category}
                    </Badge>
                  </div>
                </CardHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {trend.hashtags.map((hashtag, idx) => (
                        <Badge key={idx} variant="outline" className="text-xs border-orange-200 text-amber-800">
                          {hashtag}
                        </Badge>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-sm text-amber-700">Popular on:</span>
                    ...
                    <Button
                      ...
                      className="w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 shadow-sm hover:shadow-md"
                      size="sm"
                    >
        ...
        {filteredTopics.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-amber-900 mb-2">No trends found</h3>
        ...
        {/* Tips Section */}
        <div className="mt-12">
          <Card className="border-0 shadow-sm bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200">
            <CardHeader>
              <CardTitle className="flex items-center text-amber-900">
                <Sparkles className="mr-2 h-5 w-5 text-orange-600" />
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-medium mb-2 text-amber-900">💡 Content Ideas</h4>
                ...
                <div>
                  <h4 className="font-medium mb-2 text-amber-900">⚡ Best Practices</h4>
                  <ul className="text-sm text-amber-700 space-y-1">
